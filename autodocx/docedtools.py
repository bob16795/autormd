...
class parser():
    ...
    def Header_Parser(tokens):
        node = match_first(
            tokens, [parser.H1_Parser, parser.H2_Parser, parser.H3_Parser])
        if type(node) == nullNode:
            return nullNode()
        if not tokens.peek_at(node.consumed, ['NEWLINE', 'NEWLINE']):
            return nullNode()
    ...
    def Code_Parser(tokens):
        return match_first(tokens, [parser.Code_Start_Parser, parser.single_newline, parser.Parse_Text])
